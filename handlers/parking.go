...
func GetParkingIncome(c *gin.Context) {
	if c.GetString("role") != "admin" {
		ErrorResponse(c, http.StatusForbidden, "僅管理員可查看收入", "")
		return
	}

	lotIDStr := c.Query("parking_lot_id")
	if lotIDStr == "" {
		ErrorResponse(c, http.StatusBadRequest, "請提供 parking_lot_id", "")
		return
	}
	lotID, err := strconv.Atoi(lotIDStr)
	if err != nil || lotID <= 0 {
		ErrorResponse(c, http.StatusBadRequest, "無效的 parking_lot_id", "")
		return
	}
	...
	report, err := services.GetParkingLotIncome(lotID, startDate, endDate)
	if err != nil {
		ErrorResponse(c, http.StatusInternalServerError, "查詢失敗", err.Error())
		return
	}

	SuccessResponse(c, http.StatusOK, "查詢成功", report)
}
